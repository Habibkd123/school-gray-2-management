...
export default function StaffDayWisePage() {
  ...
  const generateDayWise = (id: string) => {
    const base = id.charCodeAt(id.length - 1);
    const result: string[] = [];
    for (let i = 1; i <= 31; i++) {
      if (i % 7 === 0 || i % 7 === 6) { result.push("H"); continue; }
      const val = (base + i) % 20;
  ...
  return (
    <div className="space-y-6 bg-[#F8FAFC] dark:bg-[#0F172A] min-h-screen -m-6 p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900 dark:text-white">Staff Day Wise Report</h1>
          <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mt-1">
            <span>Dashboard</span><span>/</span>
            <span className="hover:text-[#F59E0B] cursor-pointer">Reports</span><span>/</span>
            <span className="text-slate-900 dark:text-white font-medium">Staff Day Wise</span>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-border rounded-xl shadow-sm overflow-hidden text-left">
        <div className="p-5 border-b border-border flex flex-col xl:flex-row xl:items-center justify-between gap-4">
          ...
          <input type="month" value={selectedMonth} onChange={e => setSelectedMonth(e.target.value)}
            className="px-3 py-2 border border-border rounded-lg text-[13px] outline-none bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-300 cursor-pointer" />
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[13px] whitespace-nowrap">
            <thead className="bg-[#F8FAFC] dark:bg-[#0F172A] border-y border-border">
              <tr>
                <th className="px-6 py-4 text-left font-bold text-slate-700 dark:text-slate-200 sticky left-0 bg-[#F8FAFC] dark:bg-[#0F172A] z-10 shadow-[1px_0_0_0_#e2e8f0] dark:shadow-[1px_0_0_0_#1e293b]">Staff</th>
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? (
                ...
                <tr><td colSpan={32} className="px-6 py-10 text-center text-slate-400">No staff found.</td></tr>
              ) : filteredStaff.map(s => {
                const dayWise = generateDayWise(s._id);
                return (
                    ...
                    <td className="px-6 py-4 sticky left-0 bg-white dark:bg-slate-900 z-10 shadow-[1px_0_0_0_#e2e8f0] dark:shadow-[1px_0_0_0_#1e293b]">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="text-[11px] text-slate-500">{s.employee_id || s._id.slice(-6).toUpperCase()}</div>
                        </div>
                      </div>
                    </td>
