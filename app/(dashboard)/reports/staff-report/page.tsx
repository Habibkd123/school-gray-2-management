...
import {
  Search, Filter, ChevronDown, RefreshCw, Printer, Download, FileText, Loader2
} from "lucide-react";
import { useTeachers } from "../../../hooks/useTeachers";
import ReportTabs from "../ReportTabs";

export default function StaffReportPage() {
  const { teachers: staff, isLoading } = useTeachers();
  ...
  const filteredStaff = useMemo(() =>
    staff.filter(s => {
      const matchSearch =
        ...
        (s.employee_id || s._id).toLowerCase().includes(searchTerm.toLowerCase());
  ...
  return (
    <div className="space-y-6 bg-[#F8FAFC] dark:bg-[#0F172A] min-h-screen -m-6 p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900 dark:text-white">Staff Report</h1>
          <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mt-1">
            <span>Dashboard</span><span>/</span>
            <span className="hover:text-[#F59E0B] cursor-pointer">Reports</span><span>/</span>
            <span className="text-slate-900 dark:text-white font-medium">Staff Report</span>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-border rounded-xl shadow-sm overflow-hidden text-left">
        <div className="p-5 border-b border-border flex flex-col xl:flex-row xl:items-center justify-between gap-4">
          <h2 className="text-[16px] font-bold text-slate-800 dark:text-slate-100">Staff Report List</h2>
          <div className="relative">
                  ...
                  <div className="p-4 space-y-4">
                    <div className="space-y-1.5">
                      ...
                      <select value={filterDepartment} onChange={e => setFilterDepartment(e.target.value)}
                        className="w-full px-3 py-2 border border-border rounded-lg text-[13px] outline-none appearance-none bg-white dark:bg-slate-900 text-slate-600 cursor-pointer">
        ...
        <div className="p-5 flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50/50 dark:bg-slate-800/50">
          <span className="text-[13px] text-slate-500">Showing <span className="font-semibold text-slate-700 dark:text-slate-200">{filteredStaff.length}</span> staff</span>
          <div className="relative">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[13px] whitespace-nowrap">
            <thead className="bg-[#F8FAFC] dark:bg-[#0F172A] border-y border-border">
              <tr>
                <th className="px-6 py-4 text-left font-bold text-slate-700 dark:text-slate-200">Staff Member</th>
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? (
                ...
                <tr><td colSpan={5} className="px-6 py-10 text-center text-slate-400">No staff found.</td></tr>
              ) : filteredStaff.map(s => {
                const recentAtt = generateMockAttendance(s._id);
                return (
                  <tr key={s._id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap items-center gap-2">
                        <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center font-bold text-primary text-[12px] flex-shrink-0">{s.name.charAt(0)}</div>
                        <div>
                          <div className="font-semibold text-[#0F172A] dark:text-slate-100">{s.name}</div>
                          <div className="text-[11px] text-slate-500">{s.employee_id || "—"}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{s.department || "—"}</td>
                    <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{s.qualification || "—"}</td>
                    <td className="px-6 py-4">
