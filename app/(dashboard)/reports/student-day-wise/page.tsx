...
export default function StudentDayWiseReportPage() {
  ...
  const generateDayWiseAttendance = (id: string) => {
    ...
    const result = [];
    for (let i = 1; i <= 31; i++) {
      // 10% chance of weekend (mock)
      if (i % 7 === 0 || i % 7 === 6) {
        result.push("H"); // Holiday
        continue;
      }
      const val = (base + i) % 20;
      if (val < 15) result.push("P");
    ...
    <div className="space-y-6 bg-[#F8FAFC] dark:bg-[var(--sidebar-bg)] min-h-screen -m-6 p-6">
      ...
      <div className="bg-white dark:bg-slate-900 border border-border rounded-xl shadow-sm overflow-hidden text-left">
        ...
        <div className="p-5 flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50/50 dark:bg-slate-800/50">
          <span className="text-[13px] text-slate-500 dark:text-slate-400">Showing <span className="font-semibold text-slate-700 dark:text-slate-200">{filteredStudents.length}</span> students</span>
          <div className="relative">
          ...
          <table className="w-full text-[13px] whitespace-nowrap">
            ...
            <tbody className="divide-y divide-border">
              {classesLoading ? (
                ...
                <tr><td colSpan={32} className="px-6 py-10 text-center text-slate-400">No students found.</td></tr>
              ) : filteredStudents.map(s => {
                const dayWise = generateDayWiseAttendance(s._id);
                return (
                    ...
                    <td className="px-6 py-4 sticky left-0 bg-white dark:bg-slate-900 z-10 shadow-[1px_0_0_0_#e2e8f0] dark:shadow-[1px_0_0_0_#1e293b]">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="text-[11px] text-slate-500 dark:text-slate-400">{s.admission_no || s._id.slice(-6).toUpperCase()}</div>
                        </div>
                      </div>
                    </td>
                    {dayWise.map((status, i) => (
                      <td key={i} className="px-2 py-4 text-center">
                        <div className={`w-7 h-7 rounded mx-auto flex items-center justify-center text-[11px] font-bold ${status === "P" ? "bg-emerald-100 text-emerald-700" :
