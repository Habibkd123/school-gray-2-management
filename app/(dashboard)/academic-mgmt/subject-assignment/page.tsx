    ...
    <div className="space-y-6">
      ...
      <Modal isOpen={isAddOpen} onClose={() => { setIsAddOpen(false); resetForm(); }} title="Assign Subject">
        <form onSubmit={handleAdd} className="space-y-5 text-left">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] font-semibold text-foreground dark:text-slate-100">Subject <span className="text-red-500">*</span></label>
            <div className="relative">
      ...
      <Modal isOpen={isDeleteOpen} onClose={() => setIsDeleteOpen(false)} title="Remove Assignment">
        <div className="space-y-5 text-left">
          <p className="text-[14px] text-slate-600 dark:text-slate-300">
            Remove <span className="font-bold text-primary">{selected?.subject_master_id?.name}</span> from{" "}
            <span className="font-bold text-foreground dark:text-white">{selected?.class_id?.name}</span>
