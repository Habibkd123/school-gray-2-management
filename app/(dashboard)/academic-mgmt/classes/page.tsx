...
export default function AcademicClassesPage() {
  ...
  const columns: ColumnDef<ApiClass>[] = React.useMemo(() => {
    const cols: ColumnDef<ApiClass>[] = [
      ...
      { header: "Class Name", accessorKey: "name", render: (c) => <span className="font-bold text-[#F59E0B]">{c.name}</span> },
      ...
      <div className="bg-white dark:bg-slate-900 border border-border rounded-xl card-shadow">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-3 text-red-500">
            ...
            <button onClick={() => doFetch()} className="px-4 py-2 text-[13px] font-bold bg-[#F59E0B] hover:bg-[#D97706] text-white rounded-lg transition-colors">Retry</button>
          </div>
        ) : classes.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 gap-3 text-slate-400">
            <BookOpen className="w-10 h-10 opacity-30" />
            <p className="text-[14px] font-medium">No classes found</p>
            {isAdmin && <button onClick={() => { resetForm(); setIsAddOpen(true); }} className="px-4 py-2 text-[13px] font-bold bg-[#F59E0B] hover:bg-[#D97706] text-white rounded-lg transition-colors">Add First Class</button>}
          </div>
        ) : (
          <>
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1)
                    ...
                    .map((p, i) => p === "..." ? (
                      ...
                      <button key={p} onClick={() => handlePageChange(p as number)}
                        className={`w-8 h-8 rounded-lg text-[13px] font-bold transition-colors ${p === currentPage ? "bg-[#F59E0B] text-white shadow-sm" : "border border-border text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800"}`}>
                        {p}
                      </button>
                    ))}
