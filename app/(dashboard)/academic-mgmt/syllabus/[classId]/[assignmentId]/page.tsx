  ...
  const renderTreeNode = (node: SyllabusNode, level: number) => {
    ...
    return (
      <div key={node.id} className="space-y-1 mt-1 text-left font-normal animate-in fade-in duration-200">
        <div
          className={`flex items-start md:items-center justify-between p-3 border border-border rounded-xl transition-all duration-200 ${node.type === "unit" ? "bg-slate-50/50 dark:bg-slate-900/60 font-bold border-l-4 border-l-indigo-500 shadow-sm" :
            ...
            <div className="p-5 border-b border-border bg-slate-50/50 dark:bg-slate-800/40 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-left">
              <div>
                <h3 className="font-bold text-[16px] text-slate-900 dark:text-white flex items-center gap-2">
                  ...
                  <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold border ${isPublished ? "bg-emerald-50 text-emerald-600 dark:bg-emerald-950/20 border-emerald-250" : "bg-slate-50 text-slate-600 dark:bg-slate-800 border-border"
                    }`}>
                    ...
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <span className="text-[10px] text-slate-400 block font-sans mt-0.5">Mime: {file.mime_type || "document"}</span>
                        </div>
                      </div>
                      <a
                        href={file.file_url}
                ...
                <div key={index} className="flex items-center justify-between pt-3 text-xs font-semibold">
                  <div className="text-left space-y-1">
                    <p className="font-bold text-slate-900 dark:text-white flex items-center gap-2">
                      <span className="font-sans bg-slate-150 dark:bg-slate-855 px-2 py-0.5 rounded text-[11px]">Version {entry.version}</span>
                      {entry.title}
                    </p>
