  ...
  const FieldInput = ({ label, value, onChange, type = "text", placeholder = "" }: { label: string; value: string | number; onChange: (v: string) => void; type?: string; placeholder?: string }) => (
    <div className="flex flex-col gap-1.5 text-left">
      ...
      <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
        className="px-3.5 py-2.5 border border-border rounded-lg bg-white dark:bg-slate-900 text-[13px] text-slate-900 dark:text-white outline-none focus:border-[#F59E0B]/50 focus:ring-2 focus:ring-[#F59E0B]/10 transition-all shadow-sm" />
    </div>
  );
                  ...
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-2 cursor-pointer" onClick={e => { e.stopPropagation(); setViewBus(bus); }}>
                      <div className="w-8 h-8 rounded-lg bg-amber-50 dark:bg-amber-900/20 flex items-center justify-center">
                        <Bus className="w-4 h-4 text-[#F59E0B]" />
                      </div>
                      <div>
                        <div className="font-semibold text-slate-900 dark:text-white group-hover:text-[#F59E0B] transition-colors">{bus.busNumber}</div>
          ...
          <div className="space-y-5">
            <div className="flex items-center gap-4 p-4 bg-amber-50/50 dark:bg-amber-900/10 rounded-xl border border-amber-100 dark:border-amber-900/20">
              <div className="w-14 h-14 rounded-xl bg-[#F59E0B]/10 flex items-center justify-center">
                <Bus className="w-7 h-7 text-[#F59E0B]" />
              </div>
              <div>
                <p className="text-[11px] text-slate-500 font-semibold uppercase">Bus</p>
                <h2 className="text-[18px] font-bold text-[#F59E0B]">{viewBus.busNumber}</h2>
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button onClick={() => setViewBus(null)} className="px-4 py-2 bg-[#F59E0B] hover:bg-[#D97706] text-white text-[13px] font-semibold rounded-lg transition-colors">Close</button>
            </div>
          </div>
        </Modal>
      )}
