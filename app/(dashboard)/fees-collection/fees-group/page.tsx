                ...
                <tr key={g._id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => handleEdit(g)} className="p-1.5 text-slate-400 hover:text-[#1E3A5F] transition-colors"><Edit2 className="w-4 h-4" /></button>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xl w-full max-w-md overflow-hidden flex flex-col animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              <div className="space-y-1.5">
                ...
                <input required type="text" value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800/50 border border-border rounded-lg text-[13px] outline-none focus:border-[#1E3A5F] transition-colors" placeholder="e.g. Class 10 Fees" />
              </div>
              <div className="space-y-1.5">
