...
import { useLeaveTypes, ApiLeaveType } from "../../../hooks/useLeaveTypes";
...
export default function LeaveTypePage() {
  const { leaveTypes, isLoading, createLeaveType, updateLeaveType, deleteLeaveType, fetchLeaveTypes } = useLeaveTypes();
  ...
  const [selectedLeaveType, setSelectedLeaveType] = useState<ApiLeaveType | null>(null);
  ...
  const openEditModal = (item: ApiLeaveType) => {
    setIsEditing(true);
    setSelectedLeaveType(item);
    setFormLeaveType(item.leave_type);
    setFormStatus(item.status);
    setIsModalOpen(true);
    setActionMenuId(null);
  };

  const openDeleteModal = (item: ApiLeaveType) => {
    setSelectedLeaveType(item);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isEditing && selectedLeaveType) {
      await updateLeaveType(selectedLeaveType._id, {
  ...
  const handleDeleteConfirm = async () => {
    if (selectedLeaveType) {
      setSubmitting(true);
      await deleteLeaveType(selectedLeaveType._id);
      setSubmitting(false);
    }
  ...
  const filteredData = useMemo(() => {
    let list = leaveTypes.filter(item => {
      ...
      if (activeFrom && activeTo && item.createdAt) {
    ...
    if (selectedSort === "Ascending") {
      list = [...list].sort((a, b) => a.leave_type.localeCompare(b.leave_type));
    } else if (selectedSort === "Descending") {
      list = [...list].sort((a, b) => b.leave_type.localeCompare(a.leave_type));
    ...
    return list;
  }, [leaveTypes, searchTerm, filterLeaveType, filterStatus, activeFrom, activeTo, selectedSort]);

  const pag = usePagination(filteredData, 10);

  // Unique leave types for filter dropdown
  const uniqueLeaveTypesForFilter = useMemo(() => {
    return Array.from(new Set(leaveTypes.map(item => item.leave_type)));
  }, [leaveTypes]);
    ...
    <div className="space-y-6 bg-[#F8FAFC] dark:bg-[#0F172A] min-h-screen -m-6 p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button onClick={() => fetchLeaveTypes()} className="w-9 h-9 rounded-full bg-white dark:bg-slate-900 border border-border flex items-center justify-center text-slate-500 dark:text-slate-400 hover:text-[#F59E0B] hover:bg-indigo-50 transition-colors shadow-sm cursor-pointer">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="relative">
                          ...
                          >
                            ...
                            {uniqueLeaveTypesForFilter.map((type, idx) => (
