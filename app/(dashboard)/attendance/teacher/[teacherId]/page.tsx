...
import { ArrowLeft, Calendar as CalendarIcon, CheckCircle2, AlertCircle, Loader2, Save, XCircle, FileText, Download } from "lucide-react";
...
export default function IndividualTeacherAttendancePage({ params }: { params: Promise<{ teacherId: string }> }) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/attendance/teacher/${teacherId}?academic_year=${filterYear}&startDate=${start}&endDate=${end}`, {
  ...
  useEffect(() => {
    if (teacherId && filterYear) {
      fetchData();
    }
  }, [teacherId, filterYear, filterMonth]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-left">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900 dark:text-white">Teacher Attendance Details</h1>
            <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mt-1">
              <span>Attendance</span><span>/</span>
              <span className="text-slate-900 dark:text-white font-medium">{data?.teacher?.name || "Teacher"}</span>
            </div>
          </div>
        </div>
      ...
      {loading && !data ? (
        ...
        <>
          {/* Teacher Info & Stats Card */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="bg-white dark:bg-slate-900 border border-border rounded-xl p-5 card-shadow lg:col-span-1">
              <h2 className="text-sm font-bold text-slate-800 dark:text-slate-200 mb-4 border-b border-border pb-2">Teacher Information</h2>
              <div className="space-y-3">
                <div><span className="text-xs text-slate-500">Name:</span> <span className="text-sm font-semibold block">{data?.teacher?.name}</span></div>
                <div><span className="text-xs text-slate-500">Employee ID:</span> <span className="text-sm font-semibold block">{data?.teacher?.employee_id || "-"}</span></div>
                <div><span className="text-xs text-slate-500">Designation:</span> <span className="text-sm font-semibold block">{data?.teacher?.designation || "-"}</span></div>
                <div><span className="text-xs text-slate-500">Department:</span> <span className="text-sm font-semibold block">{data?.teacher?.department || "-"}</span></div>
              </div>
            </div>
                ...
                <table className="w-full text-left text-[13px]">
                  ...
                  <tbody className="divide-y divide-border">
                    {data?.history?.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="px-4 py-8 text-center text-slate-400 font-medium">No records found for this month.</td>
                      </tr>
                    ) : (
                      data?.history?.map((record: any, index: number) => {
                        const recDate = new Date(record.date);
                        return (
                        <tr key={index} className="hover:bg-slate-50/50 dark:bg-slate-800/20">
                          <td className="px-4 py-3 font-medium text-slate-700 dark:text-slate-300">
                            {recDate.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}
                          </td>
                          <td className="px-4 py-3 font-medium text-slate-700 dark:text-slate-300">
                            {recDate.toLocaleDateString('en-GB', { weekday: 'short' })}
                          </td>
