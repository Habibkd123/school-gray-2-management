...
export default function OnlineFormPage() {
  ...
  return (
    <main className="py-20 px-4 md:px-8 max-w-3xl mx-auto min-h-[60vh]">
      ...
      <p className="text-[#1E3A5F] font-bold uppercase tracking-widest text-[12px] mb-10">Fill in your details and we will get back to you</p>
      <div className="bg-[#0F172A] p-10 rounded-sm shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <select value={formData.grade} onChange={e => setFormData({...formData, grade: e.target.value})} className="w-full bg-white/5 border border-white/20 rounded-sm px-4 py-3 text-white focus:outline-none focus:border-[#1E3A5F] transition-colors appearance-none">
          ...
          <button type="submit" disabled={status === "loading"} className="w-full flex items-center justify-center gap-2 py-4 rounded-sm bg-[#1E3A5F] text-white font-bold hover:bg-[#162C47] transition-all uppercase tracking-wide disabled:opacity-70">
